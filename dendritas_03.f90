...
v0(:,:) = 0._np
v(:,:) = 0._np
vm = 85e-3_np !mV magnitud del voltaje
dv = vm/real(nn,np)
...
call calculate_E(nn,dx,dy,v0,v,vm,E0_x,E0_y,E_x,E_y)
!Renombro la variable para tener campo en t=0
E_x(:,:) = E0_x(:,:)
E_y(:,:) = E0_y(:,:)
    ...
    do i = 1,nm
        ...
        do k = 1,m
            ...
            if (ndist.le.ndatt) then 
                ...
                !write(*,*)v(exs,eys),'no paso li+ a li0'
                !call calculate_E(nn,dx,dy,v0,v,vm,E0_x,E0_y,E_x,E_y)
                !write(*,*)E_x(exs,eys),E_y(exs,eys)
            endif
        enddo
...
Subroutine calculate_E(nnn,dxx,dyy,v00,vv,vmm,E0_xx,E0_yy,E_xx,E_yy)
integer,intent(in)                      ::nnn
real(np), intent(in)                    ::dxx,dyy,vmm
real(np),dimension(0:nnn,0:nnn),intent(in) ::v00,vv
real(np),dimension(0:nnn,0:nnn),intent(inout) ::E0_xx,E0_yy,E_xx,E_yy
integer                                 ::i,j
